...
- **Timeweb**: зеркало/фолбэк через `scripts/deploy-local.sh` и `scripts/push-env.sh` (используйте `.env.deploy`). Для бесплатного auto-failover через Nginx gateway см. `instructions/deployment/TIMEWEB_FAILOVER.md`.

📚 **Документация:**
- [`instructions/deployment/DEPLOYMENT_GUIDE.md`](./instructions/deployment/DEPLOYMENT_GUIDE.md) — полное руководство по развертыванию
- [`instructions/overview/ARCHITECTURE.md`](./instructions/overview/ARCHITECTURE.md) — архитектура проекта
- [`instructions/deployment/RAILWAY.md`](./instructions/deployment/RAILWAY.md) — специфичные настройки Railway
- [`instructions/setup/ADMIN_SETUP.md`](./instructions/setup/ADMIN_SETUP.md) — настройка админ-доступа
